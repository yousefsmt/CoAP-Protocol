...
namespace coap
{
     ...
     *
     * - CONFIRMABLE:     A reliable message that requires an acknowledgment (ACK) from the recipient.
     *                    If no ACK is received, the sender retransmits the message.
     ...
     * - CLIENT_ERROR_RESPONSE: Indicates a client-side error (e.g., 4.00 Bad Request, 4.04 Not Found).
    ...
    private:
        ...
        int addClassToCode(uint8_t& code, uint8_t three_bit_class);
        int addDetailToCode(uint8_t& code, uint8_t five_bit_detail);
